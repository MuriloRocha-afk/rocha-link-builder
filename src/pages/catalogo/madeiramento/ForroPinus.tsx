...
const MODELOS = [
  { id: "macho-femea-10", nome: "Forro Pinus Macho-Fêmea — 1cm × 10cm", desc: "Encaixe macho-fêmea. O mais pedido para forro interno.", lider: true },
  { id: "frisado-10", nome: "Forro Pinus Frisado — 1cm × 10cm", desc: "Friso central que disfarça as emendas entre réguas.", lider: false },
  { id: "macho-femea-20", nome: "Forro Pinus Macho-Fêmea — 1cm × 20cm", desc: "Régua larga: instalação mais rápida em áreas grandes.", lider: false },
];

const COMPRIMENTOS = ["2,0m", "2,5m", "3,0m", "3,5m", "4,0m"];

const ACESSORIOS = [
  { id: "meia-cana-pinus", nome: "Meia Cana Pinus — por metro", descricao: "Arremate perimetral entre o forro e a parede.", emoji: "📐", unidade: "m", categoria: "Madeiramento", fator: 1.4 },
  { id: "sarrafo-pinus-apoio", nome: "Sarrafo Pinus 2cm × 5cm — por metro", descricao: "Ripas de apoio para fixar o forro no teto.", emoji: "🪚", unidade: "m", categoria: "Madeiramento", fator: 2.5 },
  { id: "prego-polido-15x15", nome: "Prego Polido 15×15 sem Cabeça — Kg", descricao: "Fixação discreta das réguas de pinus.", emoji: "🔨", unidade: "Kg", categoria: "Fixadores", fator: 0.05 },
  { id: "anjo-verniz-pinus", nome: "Anjo Verniz Dura Mais — Natural 3,6L", descricao: "Protege o pinus contra umidade e amarelamento.", emoji: "✨", unidade: "un", categoria: "Tintas", fator: 0.05 },
];
      ...
      cabecalho={
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Réguas de pinus claro para forros internos. Vendido por m², com 10% de perda já calculado.
          </p>
          <div className="flex gap-2 mt-3 flex-wrap">
            {["Macho-fêmea", "1cm de espessura", "Aceita verniz e stain", "Pronta entrega"].map((t) => (
      ...
      <>
        ...
        <section className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="bg-gray-50 rounded-xl p-4 mb-5">
            ...
            <p className="text-gray-600 text-sm">{modelo.nome} · régua {comprimento}</p>
        ...
        <section className="bg-white rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-4">Quantidades sugeridas para {areaComPerda} m² de forro.</p>
          <div className="space-y-2">
            {ACESSORIOS.map((a) => {
              const qtd = Math.max(1, Math.ceil(areaComPerda * a.fator));
              ...
              return (
                <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-200">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-orange-600 text-xs font-semibold mt-0.5">Sugerido: {qtd} {a.unidade}</p>
                  </div>
                  <button
                  ...
                  >
                    {ok ? <><Check size={14} /> Ok</> : <><Plus size={14} /> Add</>}
                  </button>
                </div>
              );
            })}
        ...
        <ModalCotarWhatsApp aberto={modalWppAberto} onFechar={() => setModalWppAberto(false)} nomeProduto="Forro de Pinus" corpoMensagem={corpoMsg} />
      </>
    </ProdutoLayout>
  );
}
