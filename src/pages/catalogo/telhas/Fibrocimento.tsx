...
function calcularCobertura(comprimento: string, quantidade: number): number {
  ...
  if (!comp) return 0;
  const metros = parseFloat(comprimento.split(" ")[0].replace(",", ".")) / 100;
  const area = (metros - comp.sobreposicao) * comp.larguraUtil;
  return Math.round(area * quantidade * 10) / 10;
}
...
export default function Fibrocimento() {
  ...
  const handleAdicionar = () => {
    if (!pronto) return;
    adicionar({
      id: `fibro-${comprimento}-${espessura}`,
      nome: "Telha Fibrocimento INFIBRA",
      variacao: `${comprimento} · ${espessura}`,
      quantidade,
      unidade: "peças",
      categoria: "Telhas",
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="space-y-2">
            {COMPRIMENTOS.map((c) => (
              <button
                ...
                className={`w-full text-left p-3.5 rounded-xl border transition-all flex items-center justify-between relative
          ...
          <section className="bg-white rounded-2xl p-5 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3">
              {ESPESSURAS.map((e) => (
                <button
                  key={e.valor}
                  onClick={() => setEspessura(e.valor)}
                  className={`p-3 rounded-xl border text-center transition-all relative
                    ${espessura === e.valor ? "border-orange-500 bg-orange-50 ring-2 ring-orange-200" : "border-gray-200 hover:border-orange-300"}`}
                >
                  {e.lider && (
            ...
            {cobertura > 0 && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-5">
                <p className="text-sm text-blue-800">
                  <strong>Cobertura estimada: ~{cobertura} m²</strong>
                  <span className="text-blue-600 text-xs ml-1">(com sobreposição padrão de 14cm)</span>
                </p>
              </div>
            )}
          ...
          <table className="w-full text-sm">
            <tbody className="divide-y divide-gray-100">
              {[
                ...
                ["Espessuras disponíveis", "5mm, 6mm e 8mm"],
              ].map(([k, v]) => (
                <tr key={k}>
      ...
      <CrossSellModal
        ...
        produtoPrincipal="Telha Fibrocimento INFIBRA"
        relacionados={CROSS_SELL["fibrocimento"]}
      />
      ...
      <ModalCotarWhatsApp
        ...
        nomeProduto="Telha Fibrocimento INFIBRA"
